...
Laeme sisse Euroopa Sotsiaaluuringu Eesti 7. laine andmed (2014).
...
```{r}
data <- ee7 %>% 
  select(noimbro, eduyrs, gndr, brncntr, agea)
```
...
```{r harjutusylesanne}
data <- data %>% 
  mutate(noimbro_erind = ifelse(noimbro_z > 3, 1, 0) %>% as.factor())

data %>% 
  group_by(noimbro_erind) %>% 
  summarise(
    naiste_osakaal = mean(gndr - 1),
    immigr_osakaal = mean(brncntr - 1, na.rm = TRUE),
    keskmine_haridus = mean(eduyrs, na.rm = TRUE),
    mediaan_haridus = median(eduyrs, na.rm = TRUE),
    keskmine_vanus = mean(agea, na.rm = TRUE),
    mediaan_vanus = median(agea, na.rm = TRUE)
  )
```

Keskmised ja mediaanid ei näita muidugi kogu tõde, aga nende põhjal võiks öelda, et sisserändajate osakaalu erandlikult suureks pidavate vastajate seas on rohkem naisi ja sisserändajaid ja mõnevõrra rohkem vanemaid inimesi.
