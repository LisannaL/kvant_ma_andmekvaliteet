...
-   Y-teljel on samad tunnused, nagu on andmestikus, aga iga tunnuse kohta on 2 kategooriat: indiviidid, kelle on vastavas tunnuses valiidne väärtus e. andmelünka ei ole (sinine) ja indiviidid, kellel on andmelüngad (hall).

    -   Nt esimesel real on `lrscale` tunnus, mille alusel on tehtud 2 kategooriat: sinised näitavad teiste tunnuste (veergudes) jaotust, kelle lrscale tunnuses ei olnud andmelünki ja hallid näitavad nende veergudes olevate tunnuste jaotusi nende seas, kelle on lünk `lrscale` tunnuses

-   Vaadates karpdiagramme, saame võrrelda tunnuse jaotust. Tumedam joon karbi keskel on mediaan. Karbi külgede vahele jäävad 50% ehk pooled indiviidid. Vurrude pikkus näitab ülejäänud indiviidide jaotuse vahemikku. Eelneva näite põhjal: peaksime võrdlema siniseid ja halle diagramme, kui need on mingil määral erinevad, siis võime arvata, et lünklikkus on seotud näiteks vanusega `lrscale` tunnuses. Lünklikkus `lrscale` tunnuses on seotud väärtustega, mis on vanuse tunnuses. Ehk lünklikkus antud tunnuses ei ole täiesti juhuslik, vaid ta võib olla juhuslik.

-   Tulpdiagrammis näitavad tulba kõrgused indiviidide arvu, aga kuna tulbad on erinevad, siis hetkel need ei ole väga hästi võrreldavad. Parem oleks, kui tulp kokku teeks 100%.
